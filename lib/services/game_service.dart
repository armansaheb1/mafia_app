...
class GameService {
  ...
  Future<Map<String, dynamic>?> checkActivePlayer() async {
    final headers = await _getHeaders();
    
    try {
      print('🔍 Checking for active player...');
      final response = await http.get(
        Uri.parse('${_baseUrl}active-player/'),
        headers: headers,
      );

      print('🔍 Active player response: ${response.statusCode}');
      print('🔍 Active player body: ${response.body}');

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        if (data['has_active_player'] == true) {
        ...
        // بازیکن فعالی وجود ندارد
        print('❌ No active player found (404)');
        return null;
      } else {
        print('❌ Error checking active player: ${response.statusCode}');
        throw Exception('Failed to check active player: ${response.statusCode}');
      }
    } catch (e) {
